...
export const logout = (options?: SecondParameter<typeof lasiusAxiosInstance>) => {
  return lasiusAxiosInstance<unknown>({ url: `/auth/logout`, method: 'get' }, options);
};

export const getLogoutKey = () => [`/auth/logout`] as const;
...
export const useLogout = <TError = ErrorType<unknown>>(options?: {
  swr?: SWRConfiguration<Awaited<ReturnType<typeof logout>>, TError> & {
    swrKey?: Key;
    enabled?: boolean;
  };
  ...
  const swrKey = swrOptions?.swrKey ?? (() => (isEnabled ? getLogoutKey() : null));
  const swrFn = () => logout(requestOptions);
...
export const callback = (options?: SecondParameter<typeof lasiusAxiosInstance>) => {
  return lasiusAxiosInstance<unknown>({ url: `/auth/callback`, method: 'post' }, options);
};
