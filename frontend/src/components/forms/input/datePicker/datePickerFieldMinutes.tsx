...
export const DatePickerFieldMinutes: React.FC = () => {
  ...
  const { handleClickDown, handleClickUp, inputProps } = useDateFieldInput({
    ...
    setter: 'setMinutes',
    dispatch,
    defaultValue: state.date.getMinutes(),
    digits: 2,
  });
  ...
  return (
    <Grid
      sx={{
        label: 'DatePickerFieldMinutes',
        ...datePickerTimeInputStyle,
      }}
    >
      <ButtonUpDown direction="up" onClick={handleClickUp} />
      <Box>
        <Input {...inputProps} aria-label="Minutes" sx={datePickerDigitInputStyle} />
      </Box>
