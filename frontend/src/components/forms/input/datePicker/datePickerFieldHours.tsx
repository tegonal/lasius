...
export const DatePickerFieldHours: React.FC = () => {
  ...
  const { handleClickDown, handleClickUp, inputProps } = useDateFieldInput({
    ...
    setter: 'setHours',
    dispatch,
    defaultValue: state.date.getHours(),
    digits: 2,
  });
  ...
  return (
    <Grid
      sx={{
        label: 'DatePickerFieldHours',
        ...datePickerTimeInputStyle,
      }}
    >
      <ButtonUpDown direction="up" onClick={handleClickUp} />
      <Box>
        <Input {...inputProps} aria-label="Hours" sx={datePickerDigitInputStyle} />
      </Box>
