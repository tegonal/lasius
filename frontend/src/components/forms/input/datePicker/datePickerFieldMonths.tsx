...
export const DatePickerFieldMonths: React.FC = () => {
  ...
  const { handleClickDown, handleClickUp, inputProps } = useDateFieldInput({
    ...
    setter: 'setMonth',
    dispatch,
    defaultValue: getMonth(state.date) + 1,
    digits: 2,
  });
  ...
  return (
    <Grid
      sx={{
        label: 'DatePickerFieldMonths',
        ...datePickerDateInputStyle,
      }}
    >
      <ButtonUpDown direction="up" onClick={handleClickUp} />
      <Box>
        <Input {...inputProps} aria-label="Months" sx={datePickerDigitInputStyle} />
      </Box>
